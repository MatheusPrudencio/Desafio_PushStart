...
class ControllerPreencheMatriz{
    ...
    addPrimeiro(){
        const obj = this.sorteiaObj(this.pMatriz.array.array)
        const posicao = {
            y : 0,
            x : 0
        }
    ...
    preencheMatrizIntermediaria(matriz, pai){
        ...
        if ( possiveisObjs.length === 0 ){
            ...
            this.arraySubMatrizes.push({'posicao':matriz,
                                        'subMatriz':obj,
                                        'pai': pai})
            console.log(pMatrizParcial.matriz)
            if (pMatrizParcial.objetos.length >= 2){
                const subMatriz = this.retornaMaiorEMenorSubMatriz(pMatrizParcial.objetos)
                //subMatriz.menor.xInicial = subMatriz.menor.xInicial + obj.x
                //subMatriz.menor.xFinal = subMatriz.menor.xFinal + obj.x
                //subMatriz.maior.xInicial = subMatriz.maior.xInicial + obj.x
                //subMatriz.maior.xFinal = subMatriz.maior.xFinal + obj.x
                ...
                //pMatrizParcial[0].xInicial = matriz.xInicial + obj.x
                //console.log(pMatrizParcial.objetos[0])
